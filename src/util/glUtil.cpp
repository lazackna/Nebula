...
void drawCube() {
    static nebula::Vao cubeVao;
    static nebula::Vbo cubeVbo;
    if (!cubeVao) {
        float verts[] = {
                // back face
        ...
        std::vector<nebula::Vertex> vertices;

        for (int i = 0; i < sizeof(verts) / sizeof(float); i += 8) {
            vertices.push_back(nebula::Vertex::PTN({verts[i], verts[i + 1], verts[i + 2]}, {verts[i + 6], verts[i + 7]},
                                                   {verts[i + 3], verts[i + 4], verts[i + 5]}));
        }

        cubeVao = nebula::VAO::create();
        cubeVbo = nebula::VBO::create(vertices);
